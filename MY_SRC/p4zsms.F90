MODULE p4zsms
   ...
   !! $Id: p4zsms.F90 14276 2021-01-07 22:09:56Z aumont $ 
...
CONTAINS
   ...
   SUBROUTINE p4z_sms( kt )
      ...
      !
      rfact = r2dttrc  ! time step of PISCES
      !
      ...
      DO jnt = 1, nrdttrc          ! Potential time splitting if requested
         !
         CALL p4z_bio( kt, jnt )   ! Biological SMS
         CALL p4z_lys( kt, jnt )   ! Compute CaCO3 saturation and dissolution
         CALL p4z_sed( kt, jnt )   ! Surface and Bottom boundary conditions
         CALL p4z_flx( kt, jnt )   ! Compute surface air-sea fluxes
         !
   ...
   SUBROUTINE p4z_rst( kt, cdrw )
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN
         ...
         IF( iom_varid( numrtr, 'Consfe3', ldstop = .FALSE. ) > 0 ) THEN
            CALL iom_get( numrtr, jpdom_autoglo, 'Consfe3' , consfe3(:,:,:)  )
         ELSE
            consfe3(:,:,:) = 0._wp
         ENDIF
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN
         ...
         IF( kt == nitrst ) THEN
            ...
            IF(lwp) WRITE(numout,*) '~~~~~~~'
         ENDIF
         CALL iom_rstput( kt, nitrst, numrtw, 'PH', hi(:,:,:) )           ! pH
         CALL iom_rstput( kt, nitrst, numrtw, 'Silicalim', xksi(:,:) )    ! Si 1/2 saturation constant
         CALL iom_rstput( kt, nitrst, numrtw, 'Silicamax', xksimax(:,:) ) ! Si max concentration
   ...
   SUBROUTINE p4z_chk_mass( kt )
      ...
      IF( iom_use( "pfertot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         zwork(:,:,:) =   trn(:,:,:,jpfer) + trn(:,:,:,jpnfe) + trn(:,:,:,jpdfe)   &
            &         +   trn(:,:,:,jpbfe) + trn(:,:,:,jpsfe)                      &
            &         + trn(:,:,:,jpzoo) * feratz + trn(:,:,:,jpmes) * feratm
         !
