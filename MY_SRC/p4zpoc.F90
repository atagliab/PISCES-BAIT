MODULE p4zpoc
   ...
   LOGICAL , PUBLIC ::   ln_dustrfe
   LOGICAL , PUBLIC ::   ln_litho
   REAL(wp), PUBLIC ::   remfe
   ...
   !! $Id: p4zpoc.F90 13233 2020-07-02 18:34:16Z aumont $ 
...
CONTAINS
   ...
   SUBROUTINE p4z_poc( kt, knt )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk)   :: zfafs, zfafb
      REAL(wp), DIMENSION(jpi,jpj,jpk,jcpoc) :: alphag
      REAL(wp) :: zdust 
      REAL(wp), DIMENSION(jpi,jpj,jpk)   ::  frac
      !!---------------------------------------------------------------------
      !
     ...
     IF( ln_litho ) THEN
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  frac(ji,jj,jk) = ( trb(ji,jj,jk,jplfa) / ( trb(ji,jj,jk,jplfe) + trb(ji,jj,jk,jplfa) &
                  &              + trb(ji,jj,jk,jpbfe) + trb(ji,jj,jk,jpsfe) + rtrn ) ) 
               ENDDO
            ENDDO
         ENDDO
     ENDIF
...
! account for role of dust in retarding solubilisation of Fe from sFe
      IF ( ln_bait ) THEN
      ...
      ELSE
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ! POC disaggregation by bacterial activity. It is a function
                  ! of the mean lability and of temperature. Shrinking of 
                  ! particles due to the bacterial activity is also included 
                  ! ----------------------------------------------------------
                  zremig = zremigoc(ji,jj,jk) * xstep * tgfunc(ji,jj,jk)
...
! account for role of dust in retarding solubilisation of Fe from sFe
    ...
    ENDIF
     ...
     IF( ln_p4z ) THEN
        ! Standard PISCES part
        DO jk = 1, jpkm1
           DO jj = 1, jpj
              DO ji = 1, jpi
                 IF (tmask(ji,jj,jk) == 1.) THEN
                    ! POC solubilisation by bacterial activity. It is a function
...
! account for role of dust in retarding solubilisation of Fe from sFe
      ...
      ENDIF
      IF ( ln_dustrfe ) THEN
                  zofer = zofer * ( 1. - ( zdust / ( zdust + kdust ) ) )
      ENDIF
                  zofer = zofer * remfe
                    ! Update of the TRA arrays
...
! account for role of dust in retarding solubilisation of Fe from sFe
     ...
     IF( lk_iomput ) THEN
        IF( knt == nrdttrc ) THEN
          ...
          CALL iom_put( "REMAFS",  zfafs(:,:,:)  * tmask(:,:,:)  * 1.e+9 * zrfact2 )  ! Remineralisation of small authigenic Fe
          CALL iom_put( "REMAFB",  zfafb(:,:,:)  * tmask(:,:,:)  * 1.e+9 * zrfact2 )  ! Remineralisation of big authigenic Fe
        ENDIF
     ENDIF
