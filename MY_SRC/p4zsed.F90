...
CONTAINS
   ...
   SUBROUTINE p4z_sed( kt, knt )
      ...
      REAL(wp), DIMENSION(jpi,jpj    ) :: zsedfe, zsedlfe, zsedafe
      !!---------------------------------------------------------------------
      !
      ...
      IF( ln_river ) THEN
         ...
         IF (ln_ligand) THEN
            DO jj = 1, jpj
               DO ji = 1, jpi
                  DO jk = 1, nk_rnf(ji,jj)
                     tra(ji,jj,jk,jplgw) = tra(ji,jj,jk,jplgw) +  (rivdic(ji,jj) * 5.e-5) * lgw_ratr * rfact2
                  ENDDO
               ENDDO
            ENDDO
         ENDIF
...
!
      ...
      ! Loss of particulate organic carbon and Fe to the sediments
       ...
       DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            zsedfe(ji,jj) = ( trb(ji,jj,ikt,jpsfe) * zws3 ) + ( trb(ji,jj,ikt,jpbfe) * zws4 )
         END DO
      END DO
      !
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            zsedlfe(ji,jj) = ( trb(ji,jj,ikt,jplfe) * zws3 ) + ( trb(ji,jj,ikt,jplfa) * zws4 )
      IF ( ln_feauth ) THEN
            ...
            zsedafe(ji,jj) = ( trb(ji,jj,ikt,jpafs) * zws3 ) + ( trb(ji,jj,ikt,jpafb) * zws4 )
      ENDIF
         END DO
      END DO
      ENDIF
...
!            IF( iom_use("Sdenit" ) ) CALL iom_put( "Sdenit", sdenit (:,:) * zfact * rno3 ) ! Denitrification in the sediments
            ...
            IF( iom_use("SedFe" ) ) CALL iom_put( "SedFe", zsedfe(:,:) * zfact ) ! loss of Fe
            IF( iom_use("SedLFe" ) ) CALL iom_put( "SedLFe", zsedlfe(:,:) * zfact ) ! loss of Fe
            IF( iom_use("SedAFe" ) ) CALL iom_put( "SedAFe", zsedafe(:,:) * zfact ) ! loss of Fe
         ENDIF
      ENDIF
      !
