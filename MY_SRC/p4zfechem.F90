...
!         ztotlig(:,:,:) =  0.09 * 0.667 * trb(:,:,:,jpdoc) * 1E6 + ligand * 1E9 + MAX(0., chemo2(:,:,:) - trb(:,:,:,jpoxy) ) / 400.E-6
      IF( ln_ltonly ) THEN
         ztotlig(:,:,:) = ( docl * ( trb(:,:,:,jpdoc) * 1e6 ) ) + ( 0.002 * ( trb(:,:,:,jpoxy) * 1e6 ) )
         ztotlig(:,:,:) =  MAX( 1.8, ztotlig(:,:,:) )
...
!               IF ( ln_bait .AND. ln_dyncol ) THEN
               ...
               ! assumes constant stickiness factor for DOC of 0.3 or dynamic
               ! stickiness set in p4zsbc
               ! ----------------------------------------------------------------
               IF( ln_bait .AND. ln_nocfel ) THEN
               ...
               ENDIF
               !
               IF ( .NOT. ln_dyncol ) THEN ! if we fix colloids as 50% use below agg rate
               !! colloidal shunt associated with small particles and DOC
                  zlam1a   = ( 12.0  * 0.3 * trb(ji,jj,jk,jpdoc) + 9.05  * trb(ji,jj,jk,jppoc) ) * xdiss(ji,jj,jk)    &
                      &    + ( 2.49  * trb(ji,jj,jk,jppoc) )     &
                      &    + ( 127.8 * 0.3 * trb(ji,jj,jk,jpdoc) + 725.7 * trb(ji,jj,jk,jppoc) )
               zaggdfea = zlam1a * xstep * zfecoll
               ENDIF
               !! colloidal shunt associated with large particles
               zlam1b   = ( 1.940 * xdiss(ji,jj,jk) + 1.37 ) * trb(ji,jj,jk,jpgoc)
               IF( ln_bait .AND. ln_nocfel ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               ...
               IF ( .NOT. ln_nocfel ) THEN ! if no cfel then colloidal derived
                  ...
                  ENDIF
               ELSE
               ! If not cfel, then any lgw complexed with Fe is by definition
               ! soluble and not colloidal - so subtract
                  zligco   = clgw * max( 0., trb(ji,jj,jk,jplgw) - zFeL1(ji,jj,jk) )
               ENDIF
               zlam1b   = ( 1.940 * xdiss(ji,jj,jk) + 1.37 ) * trb(ji,jj,jk,jpgoc)
