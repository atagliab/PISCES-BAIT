MODULE trcnam_pisces
   ...
   !! $Id: trcnam_pisces.F90 12537 2020-03-11 15:02:54Z aumont $ 
...
CONTAINS
   ...
   SUBROUTINE trc_nam_pisces
      ...
      !!                     ***  trc_nam_pisces  ***  
      !!
      !! ** Purpose :  read PISCES namelist. The parameters read here activate
      !!               different functionalities of PISCES : PISCES, PISCES-QUOTA
      !!               LOBSTER, variable ligands, sediment module
      !!
      ...
      !
      REWIND( numnatp_ref )              ! Namelist nampismod in reference namelist : Pisces variables
      READ  ( numnatp_ref, nampismod, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nampismod in reference namelist' )
      REWIND( numnatp_cfg )              ! Namelist nampismod in configuration namelist : Pisces variables
