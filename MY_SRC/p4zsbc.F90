MODULE p4zsbc
   ...
   REAL(wp), PUBLIC ::   sollfe
   REAL(wp), PUBLIC ::   tsollfe
   REAL(wp), PUBLIC ::   flfe
   REAL(wp), PUBLIC ::   lgw_ratd
   REAL(wp), PUBLIC ::   lgw_ratr
   ...
   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sf_solfe     ! structure of input
   ...
   INTEGER             ::   ntimes_solfe

   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   dust  , solub    !: dust fields
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   solfe
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: p4zsbc.F90 14276 2021-01-07 22:09:56Z aumont $ 
...
CONTAINS
   ...
   SUBROUTINE p4z_sbc( kt )
      ...
      IF( ln_bait ) THEN
         ...
         ENDIF
         IF( kt == nit000 .OR. ( kt /= nit000 .AND. ntimes_solfe > 1 ) ) THEN
            CALL fld_read( kt, 1, sf_solfe )
            solfe(:,:) = MAX( rtrn, sf_solfe(1)%fnow(:,:,1) ) * ( 1.0 - fr_i(:,:) )
         ENDIF
      ELSE
      IF( ln_dust ) THEN
         IF( kt == nit000 .OR. ( kt /= nit000 .AND. ntimes_dust > 1 ) ) THEN
   ...
   SUBROUTINE p4z_sbc_init
      ...
      NAMELIST/nampissbc/cn_dir, sn_dust, sn_solfe, sn_solub, sn_riverdic, sn_riverdoc, sn_riverdin, sn_riverdon,     &
        ...
        &                hratio, lgw_rath, sollfe, tsollfe, flfe, lgw_ratd, lgw_ratr
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampissbc in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         ENDIF
         IF( ln_bait ) THEN
            WRITE(numout,*) ' solubility assumed for lithogenic Fe                sollfe =',sollfe
            WRITE(numout,*) ' timescale for lfe solubility                       tsollfe=',tsollfe
            WRITE(numout,*) ' fraction of mineral dust to lfe flfe=',flfe
         ENDIF
         WRITE(numout,*) '    Equimolar supply of lgw with solfe lgw_ratd=',lgw_ratd
         WRITE(numout,*) '    Equimolar supply of lgw with rivers lgw_ratr=',lgw_ratr
      END IF
...
!!!! BAIT model - read soluble Fe input
            IF( ln_bait ) THEN

         IF(lwp) WRITE(numout,*) '    initialize solFe input from atmosphere '
         IF(lwp) WRITE(numout,*) '    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ '
         !
         ALLOCATE( solfe(jpi,jpj) )    ! allocation
         !
         ALLOCATE( sf_solfe(1), STAT=ierr )           !* allocate and fill sf_sst (forcing structure) with sn_sst
         IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'p4z_sed_init: unable to allocate sf_solfe structure' )
         !
         CALL fld_fill( sf_solfe, (/ sn_solfe /), cn_dir, 'p4z_sed_init', 'Atmospheric dust deposition', 'nampissed' )
                                   ALLOCATE( sf_solfe(1)%fnow(jpi,jpj,1)   )
         IF( sn_solfe%ln_tint )     ALLOCATE( sf_solfe(1)%fdta(jpi,jpj,1,2) )
         !
         IF( Agrif_Root() ) THEN   !  Only on the master grid
